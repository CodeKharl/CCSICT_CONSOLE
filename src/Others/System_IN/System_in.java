...
public class System_in implements Shortcuts, Animate, ConsoleClear {
  ...
  protected File ASSESSMENT_FILE =
      new File("data\\Students Assessment\\" + accountNo + "_Assessment.txt");
  protected final File ACCOUNT_INFO_FILE =
      new File("data\\Accounts_info.dat"); // data file that saves the username, password, and
  // studentType of the user.
  protected final File STUDENT_INFO_FILE =
      new File(
          "data\\Students Info\\"
              + accountNo
              + "Std_Info.dat"); // stores the student's personal information and course of the
  // students.
  protected final File COURSE_INFO_FILE =
      new File("data\\Course.dat"); // stores the student's courseChoice
  protected final File PARTIALPAYMENT_TRANSACTION_FILE =
      new File("data\\Payments\\PartialPayment.dat");
  protected final File FULLPAYMENT_TRANSACTION_FILE =
      new File("data\\Payments\\FullPayment_cash.dat");
  protected final File ACCOUNT_FILE =
      new File("data\\Accounts.dat"); // file that contains accounts balance and etc.

  protected String
      line_Data; // variable that holds a line of data in a file that reads by the file reader.
  protected boolean
      isAccount_condition; // variable that holds a judgement about the conditions on file readings.

  protected static String
      getUsername() { // for using the username variable with the value in different classes outside
    // this package.
    return System_in.username;
  }

  protected static long
      getAccountNumber() { // same as previous function but for getting the account number.
  ...
  public void typeAnyCharacter(String str) { // use for holding loops in the system when printing;
    lnHorizontalCenteredText_animation(
        DEFAULT_ANIMATION_SPEED.getValueInt(), "ENTER ANY TYPE OF CHARACTER " + str);
    scan.nextLine();
  }

  public enum forDocument { // enum containing values for writing and editing file.
    NOTEPAD_MAXCOLUMN(190),
    NOTEPAD_MIDCOLUMN(NOTEPAD_MAXCOLUMN.value / 2);
  ...
  public enum Messages { // enum containing messages that uses all through the system.
    MESSAGE(null),
    INVALIDCHOICE("INVALID INPUT! PLEASE ENTER VALID CHOICE"),
    NUMFORMATEXCEPTION("ONLY NUMBERS IS APPLICABLE TO INPUT!"),
    FILLUP("NEED TO FILL UP!"),
  ...
  public
  interface StaticShortcuts { // this interface is inside this class since it is static function. It
    // can access by using the class and its own name.
    public static void sNewLines(
        int newLines) { // print a lot of newlines depending to the parameter.
      for (int i = 0; i < newLines; i++) System.out.println();
    }

    public static void sPrintWithNewLines(
        int newLines, String string) { // print newlines with texts.
      sNewLines(newLines);
      System.out.print(string);
    }

    public static void sPrintWithNewLines_alignText(
        int newLine,
        int columns,
        String
            string) { // this funcion is the same to the previous but it can print multiple newlines
      // before the text.
      for (int i = 0; i < newLine; i++) System.out.println();
      System.out.print(
          String.format(
              "%" + ((columns - string.length()) / 2) + "s%s",
              "",
              string)); /*this will print the text that have given paddings. For e.g(%75s%s). By using this string.format, the
    ...
    public static void sNewLinesHorizontalCenteredTextln(int newLines, String string) {
      int padding =
          (Animate.forAnimation_Design.DEFAULT_NO_COLUMNS.getValueInt() - string.length()) / 2;
...
interface Shortcuts {
  public Animate.forAnimation_Design DEFAULT_NO_COLUMNS =
      Animate.forAnimation_Design.DEFAULT_NO_COLUMNS;
  ...
  public Animate.forAnimation_Design SHORT_ANIMATION_SPEED =
      Animate.forAnimation_Design.SHORT_ANIMATION_SPEED;
  public Animate.forAnimation_Design DEFAULT_ANIMATION_SPEED =
      Animate.forAnimation_Design.DEFAULT_ANIMATION_SPEED;
  public Animate.forAnimation_Design LONG_ANIMATION_SPEED =
      Animate.forAnimation_Design.LONG_ANIMATION_SPEED;
  ...
  public default String stringWithTabs_newLines_spaces(
      int newLines, int tabs, int spaces, String string) {
    String textStruct = "";
  ...
  public default void printWithTabs_newLines_spaces(
      int newLine, int tabs, int spaces, String string) {
  ...
  public default void horizontalCenteredText(String string) {
    ...
    System.out.print(
        String.format(
            "%" + PADDING.getValueInt() + "s%s%" + PADDING.getValueInt() + "s", "", string, ""));
  }
...
interface Animate {
  public enum forAnimation_Design {
    ...
    DEFAULT_DELAY(600),
    LONG_DELAY(900),
    SHORT_ANIMATION_SPEED(10),
    DEFAULT_ANIMATION_SPEED(30),
    LONG_ANIMATION_SPEED(50),
    ARROW("--> "),
  ...
  private void loading(
      int columns, char[] characterAni, int steps, int delay, String partOfAnimation) {
  ...
  private void print_backToFirstPoint_currentLine_spaces(
      int columns, String string) { // have return to firstline method.
    int padding = (columns - string.length()) / 2;
  ...
  public default void lnHorizontalCenteredText_animation(int delay, String string) {
    System.out.print(
        "\n"
            + String.format(
                "%"
                    + (Animate.forAnimation_Design.DEFAULT_NO_COLUMNS.getValueInt()
                            - string.length())
                        / 2
                    + "s",
                "",
  ...
  public default void newLinesHorizontalCenteredText_animation(
      int newLines, int delay, String string) {
    for (int i = 0; i < newLines; i++) System.out.println();

    System.out.print(
        String.format(
            "%"
                + (Animate.forAnimation_Design.DEFAULT_NO_COLUMNS.getValueInt() - string.length())
                    / 2
                + "s",
            "",
            ""));
  ...
  public default void printWithNewLines_alignText_animation(
      int newLine, int delay, int columns, String string) {
    for (int i = 0; i < newLine; i++) System.out.println();
